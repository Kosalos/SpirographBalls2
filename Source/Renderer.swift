...
class Renderer: NSObject, MTKViewDelegate {
    ...
    init?(metalKitView: MTKView, _ mIdent:Int) {
        ...
        //---------------------------
        let tNames:[String] = [ "texture1","texture2","texture3","texture4","texture5","texture6","texture7" ]
        
        do {
            for t in tNames {
                do {
                    let texture:MTLTexture = try Renderer.loadTexture(device: gDevice, textureName: t)
                    textures.append(texture)
                } catch { print("texture Error: \(error)");  exit(0)  }
            }
        }
        //---------------------------
